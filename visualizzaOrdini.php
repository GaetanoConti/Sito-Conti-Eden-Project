...
if ($modalita <> 'tutti') {
    $query="select ord.id, acc.nome, acc.cognome, ord.telefono, ord.giorno, ord.fasciaoraria, ord.prezzofinale, ord.modalita from ordini ord,accounts acc where ord.modalita='$modalita' and ord.giorno >= '$datainizio' and ord.giorno <= '$datafine' and ord.cliente = acc.email order by ord.giorno, ord.fasciaoraria DESC;"; 
    $res = pg_exec($query);
